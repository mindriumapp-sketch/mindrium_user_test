...
현재 빌드에서는 서버의 `currentWeek`, `lastCompletedWeek`, 교육/이완 완료 상태를 기준으로 주차별 접근과 복습 모드를 결정합니다. 미래 주차 카드는 이전 주차 완료 전까지 잠금 상태로 표시됩니다.
...
- 8주 프로그램 카드는 서버 진행도 기준으로 현재 주차까지만 열립니다.
- Week 2 Relief, Week 4 대안적 생각, Week 6 해결 흐름의 일부 잠금은 `bool.fromEnvironment` 플래그로 제어되며 기본값은 잠금 활성화입니다.
