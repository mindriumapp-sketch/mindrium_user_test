...
class AbcGroupCharacterScreen extends StatefulWidget {
  final String? abcId;
  final String? sessionId;
  final List<dynamic> groups;

  const AbcGroupCharacterScreen({
    super.key,
...
class _AbcGroupCharacterScreenState extends State<AbcGroupCharacterScreen> {
  ...
  Future<void> _loadCharacters() async {
    try {
      ...
      // 3) unused / used로 나누기
      final List<Map<String, dynamic>> unused = [];
      final List<Map<String, dynamic>> used = [];

      for (final c in allCharacters) {
        if (usedCharacterIds.contains(c['id'])) {
          used.add(c);
        } else {
          unused.add(c);
        }
      ...
      // 5) 최종 리스트 = [셔플된 unused..., used...]
      setState(() {
        availableCharacters = [...unused, ...used];
      });
